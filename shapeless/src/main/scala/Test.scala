...
case class LabelledCounter[A <: HList, ALen <: Nat](name: String, labels: A)
    extends ProductArgs {
  def labelValuesProduct[B <: HList, BLen <: Nat, Zipped <: HList](
      labelValues: B)(
      implicit labelValuesLen: Length.Aux[B, BLen],
  ...
  def inc(d: Double)(implicit ev: ALen =:= Nat._0): Unit =
    (); //println(s"Incrementing by $d")
  def inc()(implicit ev: ALen =:= Nat._0): Unit =
    (); //println(s"Incrementing")
}

case class Counter(name: String) extends ProductArgs {
  def applyProduct[A <: HList, ALen <: Nat](labels: A)(
      implicit lubString: LUBConstraint[A, String],
